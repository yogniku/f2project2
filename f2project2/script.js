...
var arr=[{"id":1,"first_name":"Chadwick","last_name":"Ayre","email":"cayre0@cam.ac.uk","gender":"Male","img_src":"https://robohash.org/corporisquiaperiam.png?size=50x50&set=set1","class":11,"marks":18,"passing":false,"city":"Moorreesburg"},
...
function search(eventDetails){
    ...
    let searchOutput = arr.filter(student => (student.first_name.toLowerCase() === searchInput || student.last_name.toLowerCase() === searchInput
     || student.email.toLowerCase() === searchInput || student.first_name.toLowerCase()+" "+student.last_name.toLowerCase() == searchInput))
   
    while (tbody.firstChild) {
        tbody.removeChild(tbody.firstChild);
    }
    for(let studentObj of searchOutput){
        filler(studentObj)
    }
}
function filler(student){
    ...
    let cellClass = row.insertCell(3)
    let cellMarks = row.insertCell(4)
    let cellPassing = row.insertCell(5)
    let cellEmail = row.insertCell(6)
    
    cellId.innerHTML = student.id
    cellName.innerHTML = student.first_name+" "+student.last_name
    cellGender.innerHTML = student.gender
    cellClass.innerHTML = student.class
    cellMarks.innerHTML = student.marks
    cellEmail.innerHTML = student.email
    if(student.passing){
        cellPassing.innerHTML = "Passed"
    } else {
        cellPassing.innerHTML = "Failed"
    }
...
let pass = document.getElementById("passing")
